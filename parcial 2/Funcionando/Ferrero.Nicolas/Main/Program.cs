...
namespace Main
{
    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            /*cajon.EventoPrecio += new Cajon<Fruta>.CajonDelegado(cajon.Manejador);
            cajonplatanos.EventoPrecio += new Cajon<Platano>.CajonDelegado(cajonplatanos.Manejador);*/
            cajon.EventoPrecio += new CajonDelegado(cajon.Manejador);
            cajonplatanos.EventoPrecio += new CajonDelegado(cajonplatanos.Manejador);
        ...
        private static string ObtenerPreciosBD(ISerializable obj)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT * FROM PreciosFruta ", conn);
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    cad.AppendLine(string.Format("{0}-{1}-{2}", dr[0].ToString(), dr[1] ,dr[2].ToString()));
                }
                conn.Close();
                dr.Close();
            }
